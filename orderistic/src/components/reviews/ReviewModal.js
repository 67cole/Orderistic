...
function ReviewModal({ show, handleClose, element }) {
  ...
  React.useEffect(() => {
    ...
    returnDishReview(element.id).then((data) => {
      if (currentUser) {
        data = data.filter((review) => review.user_id !== currentUser.uid);
      }
    ...
    if (currentUser) {
      returnUserReview(currentUser.uid).then((data) => {
        data = data.filter((review) => review.food_id === element.id);
        data.sort((date1, date2) => date2.date - date1.date);
        setUserReviews(data);
      });
    }
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body style={{ padding: "20px 30px 10px 30px" }}>
          ...
          <Collapse in={openForm}>
            <div>
              ...
              <div style={center}>
                <Button
                  variant="dark"
                  disabled={currentRating === 0 || name === ""}
                  style={{
                    marginRight: "5px",
                    boxShadow: "none",
                    borderRadius: "10px",
                  }}
                  onClick={addCurrentReview}
                >
                  Post
                </Button>
                <Button
          ...
          {reviews.length === 0 && userReviews.length === 0 ? (
            <div style={center}>No reviews yet.</div>
          ) : (
            <div>
              <Button
                variant="outline-light"
